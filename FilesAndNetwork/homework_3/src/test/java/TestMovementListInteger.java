...
public class TestMovementListInteger {
    ...
    void testSumIncome() {
        assertEquals(1500.0, new Movements("target/test-classes/movementListInteger.csv").getIncomeSum(), DELTA);
    }

    @Test
    @DisplayName("Сумма расходов")
    void testSumExpense() {
        assertEquals(300.0, new Movements("target/test-classes/movementListInteger.csv").getExpenseSum(), DELTA);
    }
